...
inline __m128 hsum_ab(__m256 a, __m256 b) {
	...
	__m128 s = _mm_hadd_ps(_mm256_extractf128_ps(s_256, 0), _mm256_extractf128_ps(s_256, 1));
	s = _mm_hadd_ps(s, s);
	return s;
}
...
void MatchedFilterCore_AccumulatedError_AVX2(size_t x_start_index,
	...
	for (size_t i = 0; i < y.size(); ++i) {
		...
		const int chunk1 = std::min(h_size, static_cast<int>(x_size - x_start_index));
		if (chunk1 != h_size) {
		...
		const float *x_p = chunk1 != h_size ? scratch_memory.data() : &x[x_start_index];
		const float *h_p = &h[0];
		...
		for (int k = limit_by_16; k > 0; --k, h_p += 16, x_p += 16, a_p += 4) {
			...
			// Store the contents of the __m256 register into an array
			// This change from the webRTC original file is made in order to build with MSVC
			alignas(32) float s_inst_hadd_256_arr[8];
			_mm256_storeu_ps(s_inst_hadd_256_arr, s_inst_hadd_256);
			alignas(16) float e_128_arr[4];
			s_acum += s_inst_hadd_256_arr[0];
			e_128_arr[0] = s_acum - y[i];
			s_acum += s_inst_hadd_256_arr[4];
			e_128_arr[1] = s_acum - y[i];
			s_acum += s_inst_hadd_256_arr[1];
			e_128_arr[2] = s_acum - y[i];
			s_acum += s_inst_hadd_256_arr[5];
			e_128_arr[3] = s_acum - y[i];
			e_128 = _mm_loadu_ps(e_128_arr);
#else
		...
		__m128 x2_sum_128 = _mm_add_ps(_mm256_extractf128_ps(x2_sum_256, 0), _mm256_extractf128_ps(x2_sum_256, 1));
...
void MatchedFilterCore_AVX2(size_t x_start_index,
	...
	if (compute_accumulated_error) {
		return MatchedFilterCore_AccumulatedError_AVX2(x_start_index, x2_sum_threshold, smoothing, x, y, h,
		                                               filters_updated, error_sum, accumulated_error, scratch_memory);
	}
	const int h_size = static_cast<int>(h.size());
	const int x_size = static_cast<int>(x.size());
...
#if defined(_MSC_VER) && !defined(__clang__)
		// Store the contents of the __m128 register into an array
		// This change from the webRTC original file is made in order to build with MSVC
